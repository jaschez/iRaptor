...
public class RoomGenerator
{
    ...
    //Lista de salas enlazadas, el valor corresponde al índice de habitación encontrado en la lista de DungeonGenerator
    ...
    public List<List<Coord>> enemyCoords { get; private set; }
    ...
    void CalculateEnemyCoords()
    {
        if (id != 0)
        {
            enemyCoords.Add(new List<Coord>());

            for (int i = 0; i < maxEnemies; i++)
            {
                enemyCoords[0].Add(floorCoords[Random.Range(0, floorCoords.Count)]);
            }
        }
    }
    ...
    public bool OverlapsRoom(RoomGenerator another)
    {
    ...
    public List<int> GetLinkedCorridors()
    {
        List<int> linked = new List<int>();

        foreach (Room.Entry entry in entries)
        {
            if (entry.type != Room.EntryType.Central) {
                linked.Add(entry.LinkedCorridor);
            }
        }
